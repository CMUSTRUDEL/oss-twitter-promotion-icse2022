...
m_compare_attracted_verse_non_attracted_complete_random_effect = glmer(being_attracted_flag~
             log(developer_git_tenure_in_days + 1)
            + log(developer_commit_count_in_history + 1)
            + as.logical(collaborated_user_before)
            + log(developer_twitter_tenure_in_days + 1)
            ...
            ,family = binomial()
            ,data = regression_data_ace_no_outlier)
vif(m_compare_attracted_verse_non_attracted_complete_random_effect)
summary(m_compare_attracted_verse_non_attracted_complete_random_effect)
r.squaredGLMM(m_compare_attracted_verse_non_attracted_complete_random_effect)
...
ggplot(parallel_star, aes(x = relative_hour)) +                    
  ...
  geom_line(aes(y=treatment_average_star, colour="treatment"), size = 1)  + xlab('Relative Hour') + ylab('Average star')+ scale_color_manual(values = c(
    'treatment' = 'red',
    'control' = 'green')) + theme(legend.position = c(0.86, 0.90),
